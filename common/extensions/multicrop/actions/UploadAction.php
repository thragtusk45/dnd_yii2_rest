<?php
namespace common\extensions\multicrop\actions;
...
//            'maxFiles' => $this->maxFiles,
        ...
        foreach($this->imagesOptions as $option){
            if($this->minWidth === null || $option['width'] > $this->minWidth){
                $this->minWidth = $option['width'];
            }
            if($this->minHeight === null || $option['height'] > $this->minHeight){
                $this->minHeight = $option['height'];
            }
        }
    ...
    public function run()
    {
        if (Yii::$app->request->isPost) { // TODO: throw 404 exception if not post request

            if(Yii::$app->request->get('crop')){

                $coords = Yii::$app->request->post('coords');
                $option = $this->imagesOptions[Yii::$app->request->post('key')];
                $box1 = new Box($coords['w'], $coords['h']);
                $box2 = new Box($option['width'], $option['height']);
                ...
                Image::getImagine()
                    ...
                    ->save($this->getSavePath(Yii::$app->request->post('currentImage')), ['jpeg_quality' => 100, 'png_compression_level' => 9]);
            }
            else {
                ...
                if ($model->validate()) {
                    ...
                    list($origImageWidth, $origImageHeight) = getimagesize($model->file->tempName);

                    $tempPath = $this->getSavePath($model->file->name);
                    $model->file->saveAs($tempPath, false);

                    $model->file->saveAs($this->getSavePath($model->file->name, true));

                    $images = [];
                    foreach($this->imagesOptions as $key=>$option){
                        $fileName = uniqid() . '.' . $model->file->extension;
                        $images[$key] = ['file' => $fileName, 'coords' => []];

                        $k1 = $origImageWidth / $option['width'];
                        $k2 = $origImageHeight / $option['height'];

                        if($k1 <= $k2){

                            $images[$key]['coords']['x'] = 0;
                            $images[$key]['coords']['w'] = $origImageWidth;

                            $images[$key]['coords']['h'] = round($option['height'] * $k1);
                            $images[$key]['coords']['y'] = abs(round(($origImageHeight - $images[$key]['coords']['h']) / 2));

                        }
                        else{
                            $images[$key]['coords']['y'] = 0;
                            $images[$key]['coords']['h'] = $origImageHeight;

                            $images[$key]['coords']['w'] = round($option['width'] * $k2);
                            $images[$key]['coords']['x'] = abs(round(($origImageWidth - $images[$key]['coords']['w']) / 2));
                        }

                        $box = new Box($option['width'], $option['height']);

                        Image::getImagine()->open($tempPath)
                            ->thumbnail($box, ManipulatorInterface::THUMBNAIL_OUTBOUND)
                            ->save($this->getSavePath($fileName), [
                                'jpeg_quality' => 100, 'png_compression_level' => 9
                            ]);
                    }
     ...
     */
    protected function getSavePath($fileName, $original = false)
    {
        $path = ($original) ? $this->originalPath: $this->path;
