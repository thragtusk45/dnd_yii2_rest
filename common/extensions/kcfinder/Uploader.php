...
use common\extensions\kcfinder\lib\PathHelper;
use common\extensions\kcfinder\lib\FileHelper;
use common\extensions\kcfinder\lib\FastImage;
use common\extensions\kcfinder\core\types\TypeMime;
use common\extensions\kcfinder\core\types\TypeImg;
use common\extensions\kcfinder\Config;
...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    public function __construct() {
        ...
        $i18n = Yii::$app->i18n;
        $i18n->translations['extensions/kcfinder/*'] = [
            'class' => 'yii\i18n\PhpMessageSource',
            'sourceLanguage' => 'ru',
            'basePath' => '@common/extensions/kcfinder/messages',
            'fileMap' => [
                'extensions/kcfinder/kcfinder' => 'kcfinder.php',
            ],
        ];
...
//        require "config.php";
        $this->config = (new Config())->getItems();
        ...
        $sessVar = "_sessionVar";
        if (isset($this->config[$sessVar])) {

            $sessVar = $this->config[$sessVar];
        ...
        if (isset($this->config['imageDriversPriority'])) {
            $this->config['imageDriversPriority'] =
                TextHelper::clearWhitespaces($this->config['imageDriversPriority']);
        ...
        if (preg_match('/^([a-z]+)\:\/\/([^\/^\:]+)(\:(\d+))?\/(.+)\/?$/',
            ...
            $this->config['uploadDir'] = strlen($this->config['uploadDir'])
                ? PathHelper::normalize($this->config['uploadDir'])
                : PathHelper::url2fullPath("/$path");
        ...
        // SITE ROOT
            ...
            $this->config['uploadDir'] = strlen($this->config['uploadDir'])
                ? PathHelper::normalize($this->config['uploadDir'])
                : PathHelper::normalize($_SERVER['DOCUMENT_ROOT']);
            $this->typeDir = "{$this->config['uploadDir']}/{$this->type}";
        ...
        // ABSOLUTE & RELATIVE
            ...
            $this->config['uploadURL'] = (substr($this->config['uploadURL'], 0, 1) === "/")
                ? PathHelper::normalize($this->config['uploadURL'])
                : PathHelper::rel2abs_url($this->config['uploadURL']);
            $this->config['uploadDir'] = strlen($this->config['uploadDir'])
                ? PathHelper::normalize($this->config['uploadDir'])
                : PathHelper::url2fullPath($this->config['uploadURL']);
            $this->typeDir = "{$this->config['uploadDir']}/{$this->type}";
            $this->typeURL = "{$this->config['uploadURL']}/{$this->type}";
        }
    ...
    public function upload() {
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (isset($_GET['dir']) &&
                ...
                $udir = PathHelper::normalize("$dir$gdir");
            ...
            if (!strlen($message)) {
                if (!is_dir(PathHelper::normalize($dir)))
                    @mkdir(PathHelper::normalize($dir), $this->config['dirPerms'], true);

                $filename = $this->normalizeFilename($file['name']);
                $target = FileHelper::getInexistantFilename($dir . $filename);
                ...
                if (!@move_uploaded_file($file['tmp_name'], $target) &&
                    ...
                    if (preg_match('/^([a-z]+)\:\/\/([^\/^\:]+)(\:(\d+))?\/(.+)$/', $url, $patt)) {
                        ...
                        $url = $base . PathHelper::urlPathEncode($path);
                    } else
                        $url = PathHelper::urlPathEncode($url);
                }
            }
        }
    ...
    protected function normalizeDirname($dirname) {
        ...
        if (isset($this->config['_normalizeFilenames']) && $this->config['_normalizeFilenames'])
            $dirname = FileHelper::normalizeFilename($dirname);
    ...
    protected function checkUploadedFile(array $aFile=null) {
        ...
        $typePatt = strtolower(TextHelper::clearWhitespaces($this->types[$this->type]));
        ...
        elseif (preg_match('/^\*([^ ]+)(.*)?$/s', $typePatt, $patt)) {
            ...
            $class = __NAMESPACE__ . "\\core\\types\\Type" . ucfirst($type);
    ...
    protected function validateExtension($ext, $type) {
        ...
        $exts = strtolower(TextHelper::clearWhitespaces($this->config['deniedExts']));
        if (strlen($exts)) {
            $exts = explode(" ", $exts);
    ...
    protected function imageResize($image, $file=null) {
        ...
        if (!($image instanceof Image)) {
            $img = factory($this->imageDriver, $image);
    ...
    protected function makeThumb($file, $overwrite=true) {
        ...
        $thumb = PathHelper::normalize($thumb);
    ...
    protected function callBack($url, $message="") {
        $message = TextHelper::jsValue($message);
...
if (window.opener) window.close();
    ...
    protected function get_htaccess() {
        return file_get_contents(__DIR__ . "/core/upload.htaccess");
    }

    public static function t($category, $message, $params = [], $language = null)
    {
        return Yii::t('extensions/kcfinder/' . $category, $message, $params, $language);
    }
