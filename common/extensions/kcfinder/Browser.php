...
use common\extensions\kcfinder\lib\DirHelper;
...
use common\extensions\kcfinder\lib\HttpCacheHelper;
use common\extensions\kcfinder\lib\ZipFolder;
...
class Browser extends Uploader {
    ...
    public function __construct() {
        ...
        if (!$this->config['disabled'] &&
            (
                ...
                !DirHelper::isWritable($thumbsDir) ||
                (
        ...
        if (!$this->config['disabled']) {
            $files = DirHelper::content($this->config['uploadDir'], [
                'types' => "file",
                'pattern' => '/^.*\.zip$/i'
            ]);
    ...
    public function run() {
        ...
        $this->session['dir'] = PathHelper::normalize($this->session['dir']);
    ...
    protected function act_browser() {
        if (isset($_GET['dir'])) {
            ...
            if ($this->checkFilePath($dir) && is_dir($dir) && is_readable($dir))
                $this->session['dir'] = PathHelper::normalize("{$this->type}/{$_GET['dir']}");
        }
        return $this->output();
    }
    ...
    protected function act_init() {
        ...
        $dirWritable = DirHelper::isWritable("{$this->config['uploadDir']}/{$this->session['dir']}");
        $data = [
            'tree' => &$tree,
            'files' => &$files,
    ...
    protected function act_thumb() {
        ...
        if (!is_file($file) || !is_readable($file)) {
            ...
            if (in_array($type, ["gif", "jpeg", "png"]) &&
                ...
                $mime = "image/$type";
                HttpCacheHelper::file($file, $mime);
            } else
                $this->sendDefaultThumb($file);
        ...
        readfile($file);
        die();
//        HttpCacheHelper::file($file, "image/$type");
    }
    ...
    protected function act_deleteDir() {
        ...
        if (!DirHelper::isWritable($dir))
            $this->errorMsg("Cannot delete the folder.");
        $result = !DirHelper::prune($dir, false);
        if (is_array($result) && count($result))
            $this->errorMsg("Failed to delete {count} files/folders.",
                ['count' => count($result)]);
        $thumbDir = "$this->thumbsTypeDir/{$_POST['dir']}";
        if (is_dir($thumbDir)) DirHelper::prune($thumbDir);
        return true;
    }
    ...
    protected function act_rename() {
        ...
        if (!$this->config['access']['files']['rename'] ||
            ...
            !$this->checkFilename($_POST['newName']) ||
            (false === ($file = "$dir/{$_POST['file']}")) ||
            !file_exists($file) || !is_readable($file) || !FileHelper::isWritable($file)
        )
            $this->errorMsg("Unknown error.");

        if (isset($this->config['denyExtensionRename']) &&
            $this->config['denyExtensionRename'] &&
            (FileHelper::getExtension($_POST['file'], true) !==
                FileHelper::getExtension($_POST['newName'], true)
            )
        )
        ...
        if (file_exists($newName))
            $this->errorMsg("A file or folder with that name already exists.");
        $ext = FileHelper::getExtension($newName);
    ...
    protected function act_cp_cbd() {
        ...
        if (!$this->config['access']['files']['copy'] ||
            !isset($_POST['dir']) ||
            !is_dir($dir) || !is_readable($dir) || !DirHelper::isWritable($dir) ||
        ...
        foreach($_POST['files'] as $file) {
            $file = PathHelper::normalize($file);
            ...
            $ext = FileHelper::getExtension($base);
            if (!file_exists($path))
    ...
    protected function act_mv_cbd() {
        ...
        foreach($_POST['files'] as $file) {
            ...
            elseif (!is_readable($path) || !is_file($path))
                $error[] = $this->label("Cannot read '{file}'.", $replace);
            elseif (!FileHelper::isWritable($path) || !@rename($path, "$dir/$base"))
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        $target = "$dir/" . FileHelper::getInexistantFilename($filename, $dir);
    ...
    protected function getFiles($dir) {
        ...
        $return = [];
        $files = DirHelper::content($dir, ['types' => "file"]);
        if ($files === false)
            return $return;
        ...
        foreach ($files as $file) {
            ...
            $return[] = [
                ...
                'writable' => FileHelper::isWritable($file),
                'bigIcon' => $bigIcon,
                'smallIcon' => $smallIcon,
                'thumb' => $thumb,
    ...
    protected function getDirs($dir) {
        $dirs = DirHelper::content($dir, ['types' => "dir"]);
        $return = [];
        if (is_array($dirs)) {
            $writable = DirHelper::isWritable($dir);
    ...
    protected function getDirInfo($dir, $removable=false) {
        ...
        $info = [
            ...
            'writable' => $writable,
            'removable' => $removable && $writable && DirHelper::isWritable(dirname($dir)),
            'hasDirs' => $hasDirs
        ];
    ...
    protected function output($data=null, $template=null) {
        ...
        foreach ($lang as $english => $native) {
            if (substr($english, 0, 1) != "_") {
                $langString .= "'" . TextHelper::jsValue($english) . "':\"" . TextHelper::jsValue($native) . "\"";
                if (++$i < count($lang))
                    $langString .= ",";
            }
        }
        ...
        KCFinderAsset::register($this->getView());

        $this->getView()->registerJs($langString, \yii\web\View::POS_END);
